Promise.all([
    ...
    , customElements.whenDefined('wa-input')
          ...
          // PATCH count
          document.querySelector('#Page_count')?.addEventListener('change', (event) => {
            const form = document.querySelector('#PageForm_count');
            const valid = form.checkValidity();
            if(valid) {
              patchBareMetalResourceClassVal([{ name: 'softCommit', value: 'true' }, { name: 'fq', value: 'name:' + event.currentTarget.getAttribute('data-name') }]
          ...
          document.querySelector('#Page_count')?.addEventListener('focus', (event) => {
            removeGlow(event.currentTarget);
          });
          document.querySelector('#Page_count')?.addEventListener('blur', (event) => {
            const form = document.querySelector('#PageForm_count');
            const valid = form.reportValidity();
          });
