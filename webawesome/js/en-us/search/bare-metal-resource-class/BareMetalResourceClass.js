...
async function searchBareMetalResourceClass($formFilters, success, error) {
  var filters = searchBareMetalResourceClassFilters($formFilters);
  if(success == null)
    success = function( data, textStatus, jQxhr ) {};
  if(error == null)
    error = function( jqXhr, target2 ) {};

  searchBareMetalResourceClassVals(filters, target, success, error);
}

function searchBareMetalResourceClassFilters($formFilters) {
  var filters = [];
  if($formFilters) {
...
function searchBareMetalResourceClassVals(filters, target, success, error) {

  fetch(
    '/en-us/api/bare-metal-resource-class?' + filters.map(function(m) { return m.name + '=' + encodeURIComponent(m.value) }).join('&')
    , {
...
async function getBareMetalResourceClass(pk) {
  fetch(
    '/en-us/api/bare-metal-resource-class/' + name
    , {
      headers: {'Content-Type':'application/json; charset=utf-8'}
    }).then(response => {
      if(response.ok) {
...
async function patchBareMetalResourceClass($formFilters, $formValues, target, name, success, error) {
  var filters = patchBareMetalResourceClassFilters($formFilters);
  ...
  var valueCount = $formValues.querySelector('.valueCount')?.value;
  var removeCount = $formValues.querySelector('.removeCount')?.value === 'true';
  var setCount = removeCount ? null : $formValues.querySelector('.setCount')?.value;
  var addCount = $formValues.querySelector('.addCount')?.value;
  if(removeCount || setCount != null && setCount !== '')
    vals['setCount'] = setCount;
  if(addCount != null && addCount !== '')
    vals['addCount'] = addCount;
  var removeCount = $formValues.querySelector('.removeCount')?.value;
...
function patchBareMetalResourceClassFilters($formFilters) {
  var filters = [];
  if($formFilters) {
...
function patchBareMetalResourceClassVal(filters, v, val, target, success, error) {
  var vals = {};
  vals[v] = val;
  patchBareMetalResourceClassVals(filters, vals, target, success, error);
}

function patchBareMetalResourceClassVals(filters, vals, target, success, error) {
  fetch(
    '/en-us/api/bare-metal-resource-class?' + filters.map(function(m) { return m.name + '=' + encodeURIComponent(m.value) }).join('&')
    , {
      headers: {'Content-Type':'application/json; charset=utf-8'}
...
async function postBareMetalResourceClass($formValues, target, success, error) {
  var vals = {};
  if(success == null) {
    success = function( data, textStatus, jQxhr ) {
...
function postBareMetalResourceClassVals(vals, target, success, error) {
  fetch(
    '/en-us/api/bare-metal-resource-class'
    , {
      headers: {'Content-Type':'application/json; charset=utf-8'}
...
async function deleteBareMetalResourceClass(target, name, success, error) {
  if(success == null) {
    success = function( data, textStatus, jQxhr ) {
      addGlow(target, jqXhr);
      var url = data['editPage'];
  ...
  fetch(
    '/en-us/api/bare-metal-resource-class/' + encodeURIComponent(name)
    , {
      headers: {'Content-Type':'application/json; charset=utf-8'}
      , method: 'DELETE'
    }).then(response => {
...
async function putimportBareMetalResourceClass($formValues, target, name, success, error) {
  var json = $formValues.querySelector('.PUTImport_searchList')?.value;
  if(json != null && json !== '')
    putimportBareMetalResourceClassVals(JSON.parse(json), target, success, error);
}

function putimportBareMetalResourceClassVals(json, target, success, error) {
  fetch(
    '/en-us/api/bare-metal-resource-class-import'
    , {
      headers: {'Content-Type':'application/json; charset=utf-8'}
      , method: 'PUT'
...
async function deletefilterBareMetalResourceClass(target, success, error) {
  if(success == null) {
    success = function( data, textStatus, jQxhr ) {
      addGlow(target, jqXhr);
      var url = data['editPage'];
...
async function websocketBareMetalResourceClass(success) {
...
async function websocketBareMetalResourceClassInner(apiRequest) {
  ...
  if(name != null && vars.length > 0) {
    ...
    fetch(uri).then(response => {
      response.text().then(text => {
        ...
        window.listBareMetalResourceClass = JSON.parse($response.querySelector('.pageForm .listBareMetalResourceClass')?.value);
        ...
        if(inputCount) {
          document.querySelectorAll('.Page_count').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputCount.getAttribute('value');
            else
...
function pageGraphBareMetalResourceClass(apiRequest) {
    ...
    if(json['facetCounts']) {
      ...
      if(facetCounts['facetPivot'] && facetCounts['facetRanges']) {
        ...
        if(range) {
          ...
          Plotly.react('htmBodyGraphBareMetalResourceClassPage', data, layout);
        }
      }
    }
  }
...
function animateStats() {
  document.querySelector('#pageSearchVal-fqBareMetalResourceClass_time').innerText = '';
  searchPage('BareMetalResourceClass', function() {
    ...
    let animateInterval = window.setInterval(() => {
      ...
      document.querySelector('#fqBareMetalResourceClass_time').value = x;
      document.querySelector('#fqBareMetalResourceClass_time').onchange();
      searchPage('BareMetalResourceClass');
    }, speedRate);
  });
}
