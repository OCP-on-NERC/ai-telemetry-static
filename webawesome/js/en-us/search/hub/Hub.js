...
async function searchHub($formFilters, success, error) {
  var filters = searchHubFilters($formFilters);
  if(success == null)
...
function searchHubFilters($formFilters) {
  var filters = [];
  if($formFilters) {
    ...
    var filterHubName = $formFilters.querySelector('.valueHubName')?.value;
    if(filterHubName != null && filterHubName !== '')
      filters.push({ name: 'fq', value: 'hubName:' + filterHubName });
...
function searchHubVals(filters, target, success, error) {

  fetch(
    '/en-us/api/hub?' + filters.map(function(m) { return m.name + '=' + encodeURIComponent(m.value) }).join('&')
    , {
...
function suggestHubObjectSuggest($formFilters, $list, target) {
  success = function( data, textStatus, jQxhr ) {
    if($list) {
      $list.innerHTML = '';
      data['list'].forEach((o, i) => {
        var $i = document.querySelector('<i class="fa-regular fa-sitemap"></i>');
  ...
  error = function( jqXhr, target2 ) {};
  searchHubVals($formFilters, target, success, error);
}
...
async function patchHub($formFilters, $formValues, target, hubResource, success, error) {
  ...
  var valueHubName = $formValues.querySelector('.valueHubName')?.value;
  var removeHubName = $formValues.querySelector('.removeHubName')?.value === 'true';
  var setHubName = removeHubName ? null : $formValues.querySelector('.setHubName')?.value;
  var addHubName = $formValues.querySelector('.addHubName')?.value;
  if(removeHubName || setHubName != null && setHubName !== '')
    vals['setHubName'] = setHubName;
  if(addHubName != null && addHubName !== '')
    vals['addHubName'] = addHubName;
  var removeHubName = $formValues.querySelector('.removeHubName')?.value;
  if(removeHubName != null && removeHubName !== '')
    vals['removeHubName'] = removeHubName;
...
function patchHubVal(filters, v, val, target, success, error) {
  var vals = {};
  vals[v] = val;
  patchHubVals(filters, vals, target, success, error);
}

function patchHubVals(filters, vals, target, success, error) {
  fetch(
    '/en-us/api/hub?' + filters.map(function(m) { return m.name + '=' + encodeURIComponent(m.value) }).join('&')
    , {
      headers: {'Content-Type':'application/json; charset=utf-8'}
...
async function postHub($formValues, target, success, error) {
  var vals = {};
  if(success == null) {
    success = function( data, textStatus, jQxhr ) {
  ...
  var valueHubName = $formValues.querySelector('.valueHubName')?.value;
  if(valueHubName != null && valueHubName !== '')
    vals['hubName'] = valueHubName;
...
function postHubVals(vals, target, success, error) {
  fetch(
    '/en-us/api/hub'
    , {
      headers: {'Content-Type':'application/json; charset=utf-8'}
...
function putimportHubVals(json, target, success, error) {
  fetch(
    '/en-us/api/hub-import'
    , {
      headers: {'Content-Type':'application/json; charset=utf-8'}
      , method: 'PUT'
...
async function deletefilterHub(target, success, error) {
  if(success == null) {
    success = function( data, textStatus, jQxhr ) {
      addGlow(target, jqXhr);
      var url = data['editPage'];
...
async function websocketHub(success) {
  ...
  if(hubResource != null && vars.length > 0) {
    ...
    fetch(uri).then(response => {
      response.text().then(text => {
        ...
        if(vars.includes('hubName'))
          inputHubName = $response.querySelector('.Page_hubName');
        ...
        if(inputHubName) {
          document.querySelectorAll('.Page_hubName').forEach((item, index) => {
            if(typeof item.value !== 'undefined')
              item.value = inputHubName.getAttribute('value');
            else
              item.textContent = inputHubName.textContent;
          });
          addGlow(document.querySelector('.Page_hubName'));
        }
...
function pageGraphHub(apiRequest) {
    ...
    if(json['facetCounts']) {
      ...
      if(facetCounts['facetPivot'] && facetCounts['facetRanges']) {
        ...
        if(range) {
          ...
          Plotly.react('htmBodyGraphHubPage', data, layout);
        }
      }
    }
  }
...
function animateStats() {
  document.querySelector('#pageSearchVal-fqHub_time').innerText = '';
  searchPage('Hub', function() {
    ...
    let animateInterval = window.setInterval(() => {
      ...
      document.querySelector('#fqHub_time').value = x;
      document.querySelector('#fqHub_time').onchange();
      searchPage('Hub');
    }, speedRate);
  });
}
