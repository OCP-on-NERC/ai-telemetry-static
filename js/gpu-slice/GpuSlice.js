...
function searchGpuSliceVals(filters, target, success, error) {
  ...
  fetch(
    '/api/gpu-slice?' + filters.map(function(m) { return m.name + '=' + encodeURIComponent(m.value) }).join('&')
    , {
...
async function patchGpuSlice($formFilters, $formValues, target, , success, error) {
  ...
  patchGpuSliceVals( == null ? deparam(window.location.search ? window.location.search.substring(1) : window.location.search) : [{name:'fq', value:':' + }], vals, target, success, error);
}
...
async function deleteGpuSlice(target, , success, error) {
  if(success == null) {
    success = function( data, textStatus, jQxhr ) {
      addGlow(target);
      var url = data['editPage'];
  ...
  fetch(
    '/api/gpu-slice/' + encodeURIComponent()
    , {
      headers: {'Content-Type':'application/json; charset=utf-8'}
      , method: 'DELETE'
    }).then(response => {
...
async function putimportGpuSlice($formValues, target, , success, error) {
  ...
  if( != null && vars.length > 0) {
    ...
    fetch(uri).then(response => {
      response.text().then(text => {
        ...
        jsWebsocketGpuSlice(, vars, $response);
...
function pageGraphGpuSlice(apiRequest) {
  ...
  if(r) {
    ...
    if(window.mapGpuSlice) {
      ...
      window.mapGpuSlice = L.map('htmBodyGraphLocationGpuSlicePage', {closePopupOnClick: false});
      var data = [];
      var layout = {};
      ...
      if(window['DEFAULT_MAP_LOCATION'] && window['DEFAULT_MAP_ZOOM'])
        window.mapGpuSlice.setView([window['DEFAULT_MAP_LOCATION']['lat'], window['DEFAULT_MAP_LOCATION']['lon']], window['DEFAULT_MAP_ZOOM']);
      else if(window['DEFAULT_MAP_ZOOM'])
        window.mapGpuSlice.setView(null, window['DEFAULT_MAP_ZOOM']);
      else if(window['DEFAULT_MAP_LOCATION'])
        window.mapGpuSlice.setView([window['DEFAULT_MAP_LOCATION']['lat'], window['DEFAULT_MAP_LOCATION']['lon']]);
      ...
      window.mapGpuSlice.on('contextmenu', function(e) {
        var htm = '';
        if(window.listGpuSlice.length == 1) {
          window.listGpuSlice.forEach((result, index) => {
...
function patchLocation(target, location) {
  patchGpuSliceVal([{ name: 'softCommit', value: 'true' }, { name: 'fq', value: ':' + result. }]
