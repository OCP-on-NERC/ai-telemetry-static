...
function searchAiClusterVals(filters, target, success, error) {
  ...
  fetch(
    '/api/ai-cluster?' + filters.map(function(m) { return m.name + '=' + encodeURIComponent(m.value) }).join('&')
    , {
...
async function patchAiCluster($formFilters, $formValues, target, , success, error) {
  ...
  patchAiClusterVals( == null ? deparam(window.location.search ? window.location.search.substring(1) : window.location.search) : [{name:'fq', value:':' + }], vals, target, success, error);
}
...
async function deleteAiCluster(target, , success, error) {
  if(success == null) {
    success = function( data, textStatus, jQxhr ) {
      addGlow(target);
      var url = data['editPage'];
  ...
  fetch(
    '/api/ai-cluster/' + encodeURIComponent()
    , {
      headers: {'Content-Type':'application/json; charset=utf-8'}
      , method: 'DELETE'
    }).then(response => {
...
async function putimportAiCluster($formValues, target, , success, error) {
  ...
  if( != null && vars.length > 0) {
    ...
    fetch(uri).then(response => {
      response.text().then(text => {
        ...
        jsWebsocketAiCluster(, vars, $response);
...
function pageGraphAiCluster(apiRequest) {
  ...
  if(r) {
    ...
    if(window.mapAiCluster) {
      ...
      window.mapAiCluster = L.map('htmBodyGraphLocationAiClusterPage', {closePopupOnClick: false});
      var data = [];
      var layout = {};
      ...
      if(window['DEFAULT_MAP_LOCATION'] && window['DEFAULT_MAP_ZOOM'])
        window.mapAiCluster.setView([window['DEFAULT_MAP_LOCATION']['lat'], window['DEFAULT_MAP_LOCATION']['lon']], window['DEFAULT_MAP_ZOOM']);
      else if(window['DEFAULT_MAP_ZOOM'])
        window.mapAiCluster.setView(null, window['DEFAULT_MAP_ZOOM']);
      else if(window['DEFAULT_MAP_LOCATION'])
        window.mapAiCluster.setView([window['DEFAULT_MAP_LOCATION']['lat'], window['DEFAULT_MAP_LOCATION']['lon']]);
      ...
      window.mapAiCluster.on('contextmenu', function(e) {
        var htm = '';
        if(window.listAiCluster.length == 1) {
          window.listAiCluster.forEach((result, index) => {
...
function patchLocation(target, location) {
  patchAiClusterVal([{ name: 'softCommit', value: 'true' }, { name: 'fq', value: ':' + result. }]
