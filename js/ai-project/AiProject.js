...
function searchAiProjectVals(filters, target, success, error) {
  ...
  fetch(
    '/api/ai-project?' + filters.map(function(m) { return m.name + '=' + encodeURIComponent(m.value) }).join('&')
    , {
...
async function patchAiProject($formFilters, $formValues, target, , success, error) {
  ...
  patchAiProjectVals( == null ? deparam(window.location.search ? window.location.search.substring(1) : window.location.search) : [{name:'fq', value:':' + }], vals, target, success, error);
}
...
async function deleteAiProject(target, , success, error) {
  if(success == null) {
    success = function( data, textStatus, jQxhr ) {
      addGlow(target);
      var url = data['editPage'];
  ...
  fetch(
    '/api/ai-project/' + encodeURIComponent()
    , {
      headers: {'Content-Type':'application/json; charset=utf-8'}
      , method: 'DELETE'
    }).then(response => {
...
async function putimportAiProject($formValues, target, , success, error) {
...
async function websocketAiProject(success) {
  window.eventBus.onopen = function () {
    ...
    window.eventBus.registerHandler('websocketAiProject', function (error, message) {
      ...
      var  = json['id'];
      var Page = document.querySelector('#Page_')?.value;
      var pks = json['pks'];
  ...
  if( != null && vars.length > 0) {
    ...
    fetch(uri).then(response => {
      response.text().then(text => {
        ...
        jsWebsocketAiProject(, vars, $response);
...
function pageGraphAiProject(apiRequest) {
  ...
  if(r) {
    ...
    function onEachFeature(feature, layer) {
      let popupContent = htmTooltipAiProject(feature, layer);
      layer.bindPopup(popupContent);
      window.mapLayers[feature.properties.id] = layer;
    };
    if(window.mapAiProject) {
      window.geoJSONAiProject.clearLayers();
        ...
        if(result.location) {
          ...
          shapes.forEach(function(shape, index) {
            ...
            var layer = L.geoJSON(features, {
              ...
              , style: jsStyleAiProject
              , pointToLayer: function(feature, latlng) {
                return L.circleMarker(latlng, jsStyleAiProject(feature));
              }
            });
            window.geoJSONAiProject.addLayer(layer);
          });
        }
      });
    } else {
      window.mapAiProject = L.map('htmBodyGraphLocationAiProjectPage', {closePopupOnClick: false});
      var data = [];
      var layout = {};
      ...
      legend.onAdd = jsLegendAiProject;
      legend.addTo(window.mapAiProject);
      L.tileLayer('https://tile.openstreetmap.org/{z}/{x}/{y}.png', {
        maxZoom: 19,
        attribution: '&copy; <a href="http://www.openstreetmap.org/copyright">OpenStreetMap</a>'
      }).addTo(window.mapAiProject);

      if(window['DEFAULT_MAP_LOCATION'] && window['DEFAULT_MAP_ZOOM'])
        window.mapAiProject.setView([window['DEFAULT_MAP_LOCATION']['lat'], window['DEFAULT_MAP_LOCATION']['lon']], window['DEFAULT_MAP_ZOOM']);
      else if(window['DEFAULT_MAP_ZOOM'])
        window.mapAiProject.setView(null, window['DEFAULT_MAP_ZOOM']);
      else if(window['DEFAULT_MAP_LOCATION'])
        window.mapAiProject.setView([window['DEFAULT_MAP_LOCATION']['lat'], window['DEFAULT_MAP_LOCATION']['lon']]);

      layout['margin'] = { r: 0, t: 0, b: 0, l: 0 };
      window.geoJSONAiProject = L.geoJSON().addTo(window.mapAiProject);
      window.listAiProject.forEach((result, index) => {
        if(result.location) {
          var shapes = [];
          if(Array.isArray(result.location))
      ...
      window.mapAiProject.on('contextmenu', function(e) {
        var htm = '';
        if(window.listAiProject.length == 1) {
          window.listAiProject.forEach((result, index) => {
        ...
        var popup = L.popup()
            .setLatLng(e.latlng)
            .openOn(window.mapAiProject)
            .setContent(htm);
      });
      window.mapAiProject.on('popupopen', function(e) {
        if(e.popup._source) {
          var feature = e.popup._source.feature;
          jsTooltipAiProject(e, feature);
        }
      });
      const drawnItems = new L.FeatureGroup();
      window.mapAiProject.addLayer(drawnItems);
      ...
      window.mapAiProject.addControl(drawControl);
      window.mapAiProject.on(L.Draw.Event.CREATED, function (event) {
...
function patchLocation(target, location) {
  patchAiProjectVal([{ name: 'softCommit', value: 'true' }, { name: 'fq', value: ':' + result. }]
