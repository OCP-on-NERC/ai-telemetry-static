...
async function patchGpuDevice($formFilters, $formValues, target, , success, error) {
  ...
  patchGpuDeviceVals( == null ? deparam(window.location.search ? window.location.search.substring(1) : window.location.search) : [{name:'fq', value:':' + }], vals, target, success, error);
}
...
async function deleteGpuDevice(target, , success, error) {
  if(success == null) {
    success = function( data, textStatus, jQxhr ) {
      addGlow(target);
      var url = data['editPage'];
  ...
  fetch(
    '/api/gpu-device/' + encodeURIComponent()
    , {
      headers: {'Content-Type':'application/json; charset=utf-8'}
      , method: 'DELETE'
    }).then(response => {
...
async function putimportGpuDevice($formValues, target, , success, error) {
  ...
  if( != null && vars.length > 0) {
    ...
    fetch(uri).then(response => {
      response.text().then(text => {
        ...
        jsWebsocketGpuDevice(, vars, $response);
...
function pageGraphGpuDevice(apiRequest) {
  ...
  if(r) {
    ...
    if(window.mapGpuDevice) {
      ...
      window.mapGpuDevice.on('contextmenu', function(e) {
        var htm = '';
        if(window.listGpuDevice.length == 1) {
          window.listGpuDevice.forEach((result, index) => {
...
function patchLocation(target, location) {
  patchGpuDeviceVal([{ name: 'softCommit', value: 'true' }, { name: 'fq', value: ':' + result. }]
