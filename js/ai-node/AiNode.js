...
function searchAiNodeVals(filters, target, success, error) {
  ...
  fetch(
    '/api/ai-node?' + filters.map(function(m) { return m.name + '=' + encodeURIComponent(m.value) }).join('&')
    , {
...
async function patchAiNode($formFilters, $formValues, target, , success, error) {
  ...
  patchAiNodeVals( == null ? deparam(window.location.search ? window.location.search.substring(1) : window.location.search) : [{name:'fq', value:':' + }], vals, target, success, error);
}
...
async function deleteAiNode(target, , success, error) {
  if(success == null) {
    success = function( data, textStatus, jQxhr ) {
      addGlow(target);
      var url = data['editPage'];
  ...
  fetch(
    '/api/ai-node/' + encodeURIComponent()
    , {
      headers: {'Content-Type':'application/json; charset=utf-8'}
      , method: 'DELETE'
    }).then(response => {
...
async function putimportAiNode($formValues, target, , success, error) {
  ...
  if( != null && vars.length > 0) {
    ...
    fetch(uri).then(response => {
      response.text().then(text => {
        ...
        jsWebsocketAiNode(, vars, $response);
...
function pageGraphAiNode(apiRequest) {
  ...
  if(r) {
    ...
    if(window.mapAiNode) {
      ...
      window.mapAiNode = L.map('htmBodyGraphLocationAiNodePage', {closePopupOnClick: false});
      var data = [];
      var layout = {};
      ...
      if(window['DEFAULT_MAP_LOCATION'] && window['DEFAULT_MAP_ZOOM'])
        window.mapAiNode.setView([window['DEFAULT_MAP_LOCATION']['lat'], window['DEFAULT_MAP_LOCATION']['lon']], window['DEFAULT_MAP_ZOOM']);
      else if(window['DEFAULT_MAP_ZOOM'])
        window.mapAiNode.setView(null, window['DEFAULT_MAP_ZOOM']);
      else if(window['DEFAULT_MAP_LOCATION'])
        window.mapAiNode.setView([window['DEFAULT_MAP_LOCATION']['lat'], window['DEFAULT_MAP_LOCATION']['lon']]);
      ...
      window.mapAiNode.on('contextmenu', function(e) {
        var htm = '';
        if(window.listAiNode.length == 1) {
          window.listAiNode.forEach((result, index) => {
...
function patchLocation(target, location) {
  patchAiNodeVal([{ name: 'softCommit', value: 'true' }, { name: 'fq', value: ':' + result. }]
